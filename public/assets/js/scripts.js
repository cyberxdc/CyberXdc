...
const fadeCharacters = () => {
    fadeInterval = setInterval(() => {
        if (parseFloat(letters.style.opacity) > 0) {
            letters.style.opacity = (parseFloat(letters.style.opacity) - 0.1).toString();
        } else {
...
const handleMouseMove = (e) => {
    ...
    const fontSize = Number(window.getComputedStyle(letters).getPropertyValue('font-size').match(/\d+/)[0]);
...
container.onclick = (e) => {
    ...
    const clickOutside = target.className !== "dropdown-header" && target.parentElement.className !== "dropdown-header";
    if (clickOutside && dropdownOpen)
        handleCloseDropdown();
};
dropdown.onclick = () => dropdownOpen ? handleCloseDropdown() : handleOpenDropdown();
